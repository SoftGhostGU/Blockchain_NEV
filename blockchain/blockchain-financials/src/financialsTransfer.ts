...
@Info({
  title: "FinancialsTransfer",
  description: "Smart contract for Car Owners' Financials Statements",
})
export class AssetTransferContract extends Contract {
  ...
  public async InitLedger(ctx: Context): Promise<void> {
    ...
    for (const finan of financials) {
      await ctx.stub.putState(
        finan.FinancialsID,
        Buffer.from(stringify(sortKeysRecursive(finan)))
      );
      console.log(`Financials ${finan.FinancialsID} initialized`);
    }

    for (const finan of financials) {
      await ctx.stub.deleteState(finan.FinancialsID);
      console.log(`Financials ${finan.FinancialsID} deleted`);
    }

    console.log("Financials Ledger initialized");
  }
  ...
  public async UpdateAsset(
    ...
    return ctx.stub.putState(
      financialId,
      Buffer.from(stringify(sortKeysRecursive(updatedFinancial)))
    );
  }
  ...
  public async TransferAsset(
    ...
    await ctx.stub.putState(
      financialId,
      Buffer.from(stringify(sortKeysRecursive(financial)))
    );
    return oldOwner;
  }
  ...
  // GetTotalAmount returns the total amount of all assets
  @Transaction()
  @Returns("number")
  public async GetTotalAmount(ctx: Context): Promise<number> {
    const iterator = await ctx.stub.getStateByRange("", "");
    let totalAmount = 0;
    let result = await iterator.next();
    ...
    while (!result.done) {
      ...
      try {
        const record = JSON.parse(strValue) as Financials;
  ...
  // GetTotalAmountByDriver returns the total amount of all drivers' assets
  @Transaction()
  public async GetTotalAmountByDriver(ctx: Context): Promise<number> {
    const iterator = await ctx.stub.getStateByRange("", "");
    let totalAmount = 0;
    let result = await iterator.next();
